...
const spawnGitStreamMock = mock<
  () => Promise<{ text: string; exitCode: number; truncated: boolean }>
...
fix(common-scope): message 2`;
  ...
  const files = ['apps/app-scope/src/index.ts', 'packages/common-scope/src/main.ts'];
  const result = await getScopeSuggestions(files);
