...
- **Rule:** Use one package manager per workspace (per `GEMINI.md` expectations). If `pnpm` is used for scripts/tests, avoid mixing `bun install` and `pnpm install` in the same `node_modules` without a defined reconciliation process.
